...
from . import alpacaconfig as config
import pandas as pd
import logging
...
def backtest(ticker): # backtest function for an individual stock
    try:
        ...
        predictions = list(strategy.predictions)
        actual_movements = list(strategy.actual_movements)
        ...
        closes = list(strategy.data.close.array)
        cash_values = list(strategy.cash_values)
        account_values = list(strategy.account_values)
        position_sizes = list(strategy.position_sizes)
        ...
        feature_importances = list(rfc_trainer.model.feature_importances_)
        ...
        # Default empty returns in case of error
        return [], [], [], [], [], [], [], [], [], [], [], 0, {}, [], pd.DataFrame()
